...
# Baca data dari file CSV dan gabungkan
datasets = ['dataset-ayam.csv', 'dataset-ikan.csv', 'dataset-kambing.csv', 'dataset-sapi.csv',
            'dataset-tahu.csv', 'dataset-telur.csv', 'dataset-tempe.csv', 'dataset-udang.csv']

# Menggabungkan dataset dan hapus
data = pd.concat([pd.read_csv(file).dropna() for file in datasets], ignore_index=True)
# Hapus duplikat
...
def remove_symbols_and_numbers(text):
    clean_text = re.sub(r'[^a-zA-Z\s]', '', text)  # Hapus simbol dan angka
    return clean_text
...
data['Ingredients'] = data['Ingredients'].apply(lambda x: [word for word in x if word.lower() not in stop_words and word.lower() not in custom_stopwords])

# Stemming
data['Ingredients'] = data['Ingredients'].apply(lambda x: ' '.join([apply_stemming(word) for word in x]))
print(data['Ingredients'].head)
